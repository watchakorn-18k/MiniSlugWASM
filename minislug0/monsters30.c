...
//=============================================================================
// Mst 30 : G�n�rateur de squids.

/*
[mst30] Squid Generator
Zone W = 0:7:			; Largeur de la zone dans laquelle g�n�rer les squids.
Zone H = 8:15:			; Hauteur de la zone dans laquelle g�n�rer les squids.
...
{
	u8	nZoneW;			// Largeur de la zone dans laquelle g�n�rer les squids.
	u8	nZoneH;			// Hauteur de la zone dans laquelle g�n�rer les squids.
	u8	nCnt0;
	u8	nType;			// Type de monstres. 0 = Squids / 1 = M�duses.

	u16	nMstNb;			// Nb de monstres � g�n�rer.
	u8	nDist;
};
...
void Mst30_Init_SquidGenerator0(struct SMstCommon *pMst, u8 *pData)
{
	...
	nVal = GetBits(0, 7, pData, 0);			// Largeur de la zone dans laquelle g�n�rer les squids.
	pSpe->nZoneW = nVal;
	pSpe->nMstNb = 16;						// Nb de monstres � g�n�rer.
	pSpe->nDist = 0;
	nVal = GetBits(8, 15, pData, 0);		// Hauteur de la zone dans laquelle g�n�rer les squids.
	pSpe->nZoneH = nVal;
	nVal = GetBits(16, 17, pData, 0);		// Type de monstres � g�n�rer.
	pSpe->nType = nVal;
...
*/
	...
	switch (pMst->nPhase)
	{
	case e_Mst30_WaitX:			// Attend que le d�but de zone (le monstre) entre dans l'�cran.
	...
	case e_Mst30_Generate:		// On g�n�re des squids.
/*
{
//>tst
...
*/
		// Au fur et a mesure qu'on avance, on augmente le nombre de monstres � g�n�rer.
...
_31SquidDeath:
	...
	ChaserTarget_AddToList(pMst->nPosX >> 8, (pMst->nPosY >> 8));	// Ajout dans la table de vis�e des missiles.
...
//=============================================================================
// Mst 32 : L11 Scroll control : Arr�te/red�marre le scroll sur le plan des monstres en fct d'une item.
...
Nb(1) = 8:11:
Offset = 12:15:			; D�calage Y pour ceux qui viennent des c�t�s.
Scroll Stop = 16:17: No - While_gen - After_last_disp	; Stoppe le scroll pendant cette s�quence ? (Non - Pendant la g�n�ration - Jusqu'� disparition du dernier monstre).
prm0 = 18:19:			; tmp
Shot Pattern = 20:23: One_By_One - All_Together	; Type de tir : Un par un - Tous ensemble.
Shot 1st Delay = 24:27:		; D�lai avant le premier tir (* 16 frames).
Shot Delay = 28:31:		; D�lai entre les tirs (* 16 frames).
...
struct SMst33_L11Generator0
{
	...
	u8	nDrop;			// Item lach�e par le dernier monstre de la s�quence.
	u8	nNb;			// Nb de monstres � g�n�rer.
	u8	nOffset;		// D�calage Y pour ceux qui viennent des c�t�s.
	u8	nScrollStop;	// Stoppe le scroll pendant cette s�quence.
	u8	nSeqNo;			// N� de la s�quence.
	u8	nOrder;			// Incr�ment pour compter les monstres dans une s�quence.
	u8	nCnt0;
...
void Mst33_Init_L11Generator0(struct SMstCommon *pMst, u8 *pData)
{
	...
	if (nVal < 1) nVal = 1;
	if (pSpe->nType == 2) nVal *= 16;		// Multiplicateur pour les ast�ro�des.
...
s32 Mst33_Main_L11Generator0(struct SMstCommon *pMst)
{
	...
	// Cas de la disparition apr�s la derni�re disparition de monstre.
...
Drop = 4:7: Nothing - H_Machinegun - Gas_Tank -
SeqNo = 8:11:	; No de la s�quence.
Offset = 12:15:	; Offset.
Order = 16:19:			; N� d'ordre dans la s�quence (pour trouver le leader).
Shot Pattern = 20:23: One_By_One - All_Together	; Type de tir : Un par un - Tous ensemble.
Shot 1st Delay = 24:27:		; D�lai avant le premier tir (* 16 frames).
Shot Delay = 28:31:		; D�lai entre les tirs (* 16 frames).
Shot Nb = 32:35:		; Nb de tirs.
...
*/
			...
			{	// Mars UFO.
				u32	nRnd = rand();
				// On g�n�re les 3 monstres d'un coup.
				...
				pSpe->nNb = 1;		// ...Et on arr�te la g�n�ration.
			}
			else
			{	// Mars Eye / Mars TIE.
...
//			nPosX = ((rand() % (SCR_Width * 2)) - (SCR_Width / 2)) * 256;
			...
			// Reinit compteur.
			pSpe->nCnt0 = MST33_TIMER0 - (pSpe->nOffset << 2);  // Pour les ast�ro�des, on se sert de l'offset pour augmenter la fr�quence d'apparition.
			break;

		case 3:		// Big Asteroid
			gMstMisc.pSeqCount2[pSpe->nSeqNo] = 3;	// On va g�n�rer 3 ast�ro�des.
...
SeqNo = 0:3:	; No de la s�quence.
Order = 4:7:	; N� d'ordre dans la s�quence (pour trouver le leader).
Drop = 8:11: Nothing - H_Machinegun - Gas_Tank - Ammo_Box1	; H non utilis�.
...
*/
		...
		// Tous les monstres ont �t� g�n�r�s.
		if (--pSpe->nNb == 0)
		switch (pSpe->nScrollStop)
		{
		case 1:		// Stop pendant la g�n�ration.
			gpMstQuestItems[MST_QUEST_ITEM_MST32]--;		// -- sur l'item de stop.
		default:	// Pas de stop.
			return (e_MstState_Dead);
		case 2:		// Passe en phase d'attente que le dernier monstre de la s�rie disparaisse.
...
struct SMstPath
{
	...
	s16 nSpdGoal;		// Pour acc�l�rations.
	u8	nNewAngle;		// Nouvel angle � atteindre quand on tourne.
...
void Mst_PathInit(struct SMstCommon *pMst, struct SMstPath *pPath, s32 nSpdInit, u8 nAngInit)
{
	...
	pPath->nPathPosX = pMst->nPosX;	// Svg des pos de d�part.
	pPath->nPathPosY = pMst->nPosY;
	// Un bloc de chemin ?
	pMst->nAngle = nAngInit;				// Direction par d�faut.
...
//printf("Order=%d / x=%d y=%d / pathx=%d pathy=%d\n",pMst->nLoadIdx, pMst->nPosX>>12, pMst->nPosY>>12, pPath->nPathPosX>>12, pPath->nPathPosY>>12);
				// Recalage au centre du bloc de path pour les recherches � suivre.
...
// Structure sp�cifique. !!! Attention, MST41 utilise la m�me struct !!!
struct SMst34_L11MarsEye0
{
	...
	u8	nDrop;			// Item lach�e par le dernier monstre de la s�quence.
	u8	nSeqNo;			// N� de la s�quence.
	u8	nOrder;			// N� d'ordre dans la s�quence.
	u8	nHitCnt, nLife;
...
//pSpe->nDrop = gnItmTb[2];//2;	//test test test test
	nVal = GetBits(8, 11, pData, 0);
	pSpe->nSeqNo = nVal;	// N� de s�quence.
	nVal = GetBits(16, 19, pData, 0);
	pSpe->nOrder = nVal;	// N� d'ordre dans la s�quence.
	...
	// Replacement aux abords de l'�cran.
	nVal = GetBits(36, 36, pData, 0);
	pSpe->nScrollXY = nVal;
	if (pSpe->nScrollXY)
	{	// Scroll X.
...
//	DustSetMvt(gAnm_Debris_Metal0_Dust, pMst->nPosX, pMst->nPosY, -0x180, -0x380, e_Prio_DustUnder - 1, e_DustFlag_Gravity);
		...
		if (pSpe->nDrop)
		{
			// Largage d'une item quand le dernier monstre de la s�quence explose.
/*
[mst4] Weapon Capsule
Type = 0:3: S_Shotgun - H_Machinegun - F_Flamethrower - R_Rocket - Gas_Tank - Ammo_Box1
Boxed = 4:4: No - Yes		; Une caisse contient l'item ?
...
//	u32	nLeader = ((gMstMisc.pSeqLeader[pSpe->nSeqNo] & ~(0xFF << (pSpe->nOrder + 1))) == 1 << pSpe->nOrder ? 1 : 0);	// Le leader = le bit le plus � droite !
...
s32 Mst34_Main_L11MarsEye0(struct SMstCommon *pMst)
{
	...
	{
		gMstMisc.pSeqCount2[pSpe->nSeqNo]--;
		if (pSpe->nShotAnm != -1) AnmReleaseSlot(pSpe->nShotAnm);	// Lib�re le slot de tir.
		return (e_MstState_Dead);
	}
...
struct SL11SpaceRock	gpL11SpaceRocks[L11SPACEROCKS_MAX];	// Tableau g�n�ral pour stockage des cailloux.
...
struct SMst37_L11BigAsteroid0
{
	...
	u8	nDustNo;		// Pour d�calage des dusts.
};
...
{
	struct SMst37_L11BigAsteroid0	*pSpe = (struct SMst37_L11BigAsteroid0 *)pMst->pData;
	u32	nSprDisp;			// Sprite � afficher (spr + spr+1).
...
//		gMstMisc.pSeqCount2[pSpe->nSeqNo]--;
	...
	nSprDisp = e_Spr_BigAsteroid;	// Spr par d�faut.
	...
	for (i = 0; i < nSprCtcInList; i++)
	{
		u32	nSpr;
		nSpr = pnSprCtc[i];
		// Blocage des tirs (sans d�gats sur le monstre, g�r�s plus haut dans la phase e_Mst37_Fly).
		...
		if (gShoot.nInvulnerability == 0)
		{
			...
			// Appel du d�gat normal pour d�gager les cannons. > Ou pas ?
...
//				Enemy_PlayerBlowRect(&sRect1, (i == 0 ? pMst->nPosX : pSpe->nPosX2), pMst->nPosY, MST_DAMAGE_BULLET);
			...
			if (nXMax1 >= nXMin2 && nXMin1 <= nXMax2 && nYMax1 >= nYMin2 && nYMin1 <= nYMax2)
			{
				...
				{	// Si le centre du vaisseau du h�ros est dans l'ast�ro�de (fin d'invuln�rabilit�...), mort imm�diate.
				...
				// Si on s'est fait pousser hors de l'�cran, mort imm�diate.
...
struct SMst38_BigJellyfish0
{
	...
	u16	nSatPres;	// Bits pour la pr�sence des satellites. 0 = Tous morts.
	u8	pnSatLife[MST38_SAT_NB];	// Vie des diff�rents satellites.
	u8	pnSatHitCnt[MST38_SAT_NB];	// HitCnt des diff�rents satellites.
	u8	nOffset;
	u8	nWavesNb;		// Nb de cercles successifs de m�duses.

	s32	nTargetX, nTargetY;		// Cible du d�placement.
	u8	nStopWait;				// Attente apr�s d�placement.
...
void Mst38_sub_SatellitesInit(struct SMst38_BigJellyfish0 *pSpe, u32 nOffset)
{
	...
	pSpe->nSatPres = (1 << MST38_SAT_NB) - 1;	// Bits � 1 pour les pr�sents.
...
s32 Mst38_Main_BigJellyfish0(struct SMstCommon *pMst)
{
	...
	switch (pMst->nPhase)
	{
	case e_Mst38_BigJellyfish_WaitItem:		// Attend une item de qu�te avant d'appara�tre.
	...
	case e_Mst38_BigJellyfish_Stop:		// Attente.
		nTouch = 1;
		// D�clenchement du mvt (si plus de satellites, on skippe la phase de ring).
		if (MST_SHOT_COND)	// Si le h�ros n'est pas mort...
		if (--pSpe->nStopWait == 0)
		{
	...
	case e_Mst38_BigJellyfish_JfishRing:	// Anneau qui grossit/r�duit avant la charge.
		if (pSpe->nOffset < MST38_SAT_OFFSET) break;	// Si en phase de r�g�n�ration, on attend.
		// Grossissement / R�duction.
	...
	case e_Mst38_BigJellyfish_Move:		// D�placement vers sa cible.
		nTouch = 1;
		{
			s32	nIncX, nIncY;
			s32	nSub = (pSpe->nWavesNb ? 0 : 0x40);		// Pour ne pas s'arr�ter avant le joueur quand plus de cercles de m�duses.
			...
			if (ABS(nIncX) < 0x80 - nSub && ABS(nIncY) < 0x80 - nSub)
			{
				...
				pSpe->nStopWait = MST38_STOP_FRM + (pSpe->nWavesNb * MST38_WAVES_MULT);		// Attente apr�s d�placement.
			}
		}
		break;
	}

	nSpr = AnmGetImage(pMst->nAnm);		// Ma�tre.
	...
	if (pSpe->nSatPres)
	{
		...
		s32	nOffsX, nOffsY;
		u8	nAng = pSpe->nAngle;
		u32	nOffsImg = (nAng >> 2) % 10;	// Pour anim manuelle des petites m�duses.
		for (i = 0; i < MST38_SAT_NB; i++)
		{
			if (pSpe->nSatPres & (1 << i))
			{
				...
				//if (nTouch)
				if (nTouch && pSpe->nOffset > (3 * MST38_SAT_OFFSET) / 4)	// (On emp�che le toucher trop t�t � cause des bombes).
				{
					// Se prend un tir ?
...
//		pSpe->nAngle++;
		pSpe->nAngle += (pSpe->nWavesNb & 1 ? 1 : -1);		// On fait tourner le cercle de m�duses dans un sens ou dans l'autre.

		// R�-init des satellites.
		if (pSpe->nSatPres == 0)
		if (--pSpe->nWavesNb)
			Mst38_sub_SatellitesInit(pSpe, 0);
		// Si l'offset n'est pas au max, on l'incr�mente.
		if (pSpe->nOffset < MST38_SAT_OFFSET) pSpe->nOffset++;

		// Tant qu'il y a des satellites, le ma�tre ne se prend pas de d�gats.
		FireHitCheck(nSpr, pMst->nPosX, pMst->nPosY, e_ShotOrg_Player, 1);
	}
	else
	{
		// Plus de satellites, d�gats sur le ma�tre. (nTouch inutile, puisqu'on ne se retrouvera pas sans satellites ici, et les satellites font le test).
		// Se prend un tir ?
		if (Mst_ShotCheckLife2(nSpr, pMst->nPosX, pMst->nPosY, &pSpe->nHitCnt, &pSpe->nLife, e_ShotCheck_MinimizedDamage))
		{
			...
			gpMstQuestItems[pSpe->nItemGive] = 1;	// Item de qu�te.
			return (e_MstState_Dead);
		}
	}

	// Contact du corps de la m�duse avec le joueur.
	if (SprGetRect(nSpr, e_SprRectZone_RectCol, &sRect1))
	if (sRect1.nType == e_SprRect_Rect)
...
//=============================================================================
// Mst 39 : Sensor Mine (mine aimant�e).
...
s32 Mst39_Main_SensorMine0(struct SMstCommon *pMst)
{
	...
	// Si le joueur est en slug et dans le p�rim�tre, on se dirige vers le slug.
	struct SSprRect sRect1;
